...
8. ~~**派生卡 id 含槽位序号，换槽即换 id**~~（**用户裁定「改吧」，已闭合**）：`makeWieldedMeanId` 改为只吃物品编号，id 形如 `wielded_inv_42`，同一把武器在主手与副手之间移动不再解绑组合技。配套三处：
    - 槽位信息本就不靠 id 表达，卡上另有 `slotIndex` / `slotLabel` 字段承载"主手/副手/备用"标签——原记的"标签得另找来源"这一代价并未发生。
    - 旧存档兼容：历史 `primaryMeanId` 形如 `wielded_0_inv_x`，新增 `findMeanByStoredId(means, storedId)` 作为唯一解析入口（先按 id 等值找，找不到再剥掉槽位序号认一次，认不回才落灰卡），`CombatStatus.jsx` 的两处查找点（编辑弹窗回填、渲染期解析主手段）都已切过去。等值优先是为了避免物品编号本身以"数字_"开头时认错卡。
    - id 去掉槽位后，同一件物品被两个手持位同时引用（脏数据）会撞出重复 id，派生器按槽位顺序去重、主手优先。
...
10. **法术/道具/组合技卡关掉的自动增益刷新即复活**（复核后**定性为缺陷**，修法待用户裁定）：读侧其实是完整的——四类卡渲染时都走 `combatMeanUtils.js` 的 `computeLiveGains`，它现算自动增益、按 `cm.disabledAutoGainKeys` 过滤、只保留存档 `gains` 里非 auto 的条目。缺的只是写侧：`disabledAutoGainKeys` 的落盘只有 `deriveWieldedWeaponMeans.js` `buildWeaponMeanConfig` 一条，即只有派生武器卡会记住玩家关掉的增益。法术卡/道具卡/组合技卡的保存路径只写 `gains`，关掉的自动增益刷新即复活，`GainEditor` 的回填对它们是死工。**推荐修法**：这三处保存补丁各加一行 `disabledAutoGainKeys: deriveDisabledAutoGainKeys(addGains)`（`CombatStatus.jsx` 需把该函数加进 import，`combatMeanImports.test.js` 会双向校验）。备选是在这三类卡的增益编辑器里隐藏自动增益开关，但那样玩家就失去"这张卡不要某条自动增益"的能力。属既有缺口（本计划前也不持久化），**交用户裁定**。
11. **三处"用到却没在作用域里定义"的引用活在 HEAD 里**（Task 11 审查建议 6，已用独立 `no-undef` 探针核实，非误报）：同文件内有多个组件，符号定义在**另一个组件**的作用域里，编译与 `build` 都不报错，只有真正走到那条渲染分支才 ReferenceError 白屏。Task 16 收尾时逐条复核：
    ...
    - ~~`src/components/CharacterInventory.jsx:1218`~~（**用户授权后已修**；原记的 `src/pages/` 路径有误，该文件在 `components` 下）：`editingProto` 声明在 `:1019` 那层 IIFE 里，却在 IIFE 之外的 `{isEditing && (` 分支里被用到，编辑弹窗里"法器充能上限"那一行一点开就 ReferenceError 白屏。修法是把声明提到行 map 回调作用域（`isEditing && entry?.itemId ? getItemById(entry.itemId) : null`）并删掉 IIFE 内的重复声明——`isEditing` 成立即 `editingIndex === i`，故 `inv[editingIndex]` 与 `entry` 同一条目，两处取值等价。`:705` `saveEdit` 里那条同形的"法器/充能上限"判断是它的存盘侧对应物。探针已干净。
...
12. **`no-undef` 不开进仓库 lint 配置**（Task 16 最终裁定：**维持不开**）：`eslint.config.js` 目前不开 `no-undef`，Vite/esbuild 只转译不解析标识符，所以"用到未导入/未声明"永远过编译——这正是第 11 条能长期存活的土壤。全仓探下来真错极少，开启的噪声本来很低，但仍裁定不开：一是这是共享工作树，改全局 lint 配置会波及他人在改文件的提交门禁；二是当时开启后会立刻红在 `CharacterInventory.jsx:1218`——那是本计划范围外、尚未裁定的一处真错，等于用本计划的提交去替别人设卡（该处后来经用户授权已修，第二条理由随之失效，裁定仍按第一条维持）。改走"逐处修引用"的路线：本计划范围内的三处已随 Task 14 清零，剩下两处见第 11 条。**复现命令**（临时配置，不改仓库）：造一份只含 `languageOptions.globals = { browser, es2022 }` + `parserOptions.ecmaFeatures.jsx = true` + `rules: { 'no-undef': 'error' }` 的 flat config，`npx eslint --no-config-lookup --no-ignore --config <该文件> <目标文件>`。该临时配置必须放在项目目录内才能解析 `globals` 依赖；`src/**/*.test.js` 里的 `process` / `global` 需补 node globals 才不会误报。
13. ~~**`rollAllWeaponDamage` 是死代码**~~（**用户裁定「确定无用就删吧」，已闭合**）：原 `CombatStatus.jsx:1253` 定义的 86 行函数、`:3120` 塞进 `cardCtx` 的入口、`:1362` 拿它举例的误导注释三处一起清掉；注释改写为"法术/非武器伤害重击始终按规则 ×2；武器上的「暴击×」附注由武器卡读出、经计划注册表交给释放弹窗"。随函数一起失效的三个 import（`getCritDamageDiceMultiplierFromItemEntry`、`filterExtraDiceAgainstMain`、`rollDice`）一并删除，`setLastDamageRoll` 因 `rollDamageDice` 仍在用而保留。核实：`grep -rn rollAllWeaponDamage src/` 全仓已无引用，eslint 0 errors。
