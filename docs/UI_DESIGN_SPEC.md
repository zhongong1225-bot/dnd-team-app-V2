...
| 图标 | 含义 | 使用场景 |
|------|------|---------|
| 齿轮 Settings | DM 专属配置 | 效果编辑器、BUFF 默认配置、职业特性/专长数值配置等玩家不应随意改动的入口 |
| 铅笔 Pencil | DM 与玩家共同修改 | 档案、属性加值分配、名称/头像等日常可编辑内容 |

弱化要求（齿轮与铅笔均适用）：
- 默认态：灰蓝色（#667788 或 text-gray-500），无边框、无背景，尺寸 12–14px
- 悬停态：变金 #c79a42 或亮灰，可叠加极淡背景 rgba(255,255,255,0.06) 提示可点
- 禁止用金色描边、实心底色、胶囊外框强调图标本身；视觉重心始终在内容而非入口
